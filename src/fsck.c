...
int validate_superblock() {
    ...
    if (unpack_unsigned_int(sprblk->magic_number) != MAGIC) {
        printf("Invalid magic number in superblock\n");
        return -1;
    }
    if (unpack_unsigned_int(sprblk->disk_size) != size) {
        printf("Invalid size in superblock\n");
        return -1;
    }

    root_inode_num = unpack_unsigned_int(sprblk->root_inode);
    first_free_inode = unpack_unsigned_int(sprblk->first_free_inode);
    first_free_data_block = unpack_unsigned_int(sprblk->first_free_data_block);
